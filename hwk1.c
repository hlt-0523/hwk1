...
int main(int argc, char **argv){
	...
	L = createList( printNumber );	// why is this line needed? because the list L was destroyed so I have to create a new list, reinitializing L to a new list.
    ...
    destroyList(&L);
    L = createList(printString);

	printf( "\nReuse L for strings\n" );

	L = createList( printString );	// don't i need to call destroyList(L) 1st? Yes cuz I should free the memory allocated for the previous list.
